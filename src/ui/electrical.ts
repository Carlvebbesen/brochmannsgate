...
const TEMPLATE = /* html */ `
  ...
  <label class="check" title="Draws the bed, sofa, table and TV bench under the plan, so a socket lands where the furniture actually is.">
...
export class ElectricalPanel {
  ...
  private refresh() {
    ...
    this.$<HTMLInputElement>('#el-furn').checked = this.store.view.elFurniture;
    this.renderSelected();
    this.renderSummary();
  }

  private buildPhases() {
    const host = this.$('#el-phase');
    for (const p of PHASES) {
      const b = document.createElement('button');
      b.dataset.phase = p.id;
      b.textContent = p.label;
      b.title = p.hint;
      b.addEventListener('click', () => this.store.setView({ elPhase: p.id }));
      host.appendChild(b);
    }
  }
  ...
  private buildPalette() {
    ...
    for (const group of groups) {
      ...
      for (const type of EL_TYPES.filter((t) => t.group === group)) {
        ...
        b.innerHTML = `<span class="el-code">${type.code}</span><span class="el-name">${type.label}</span>`;
  ...
  private renderSelected() {
    ...
    host.innerHTML = /* html */ `
      ...
      <label class="el-field"><span>Type</span>
        <select id="el-type">${EL_TYPES.map((t) => `<option value="${t.id}" ${t.id === item.type ? 'selected' : ''}>${t.label}</option>`).join('')}</select>
      </label>
      <div class="seg el-status">${EL_STATUS.map(
        (s) => `<button data-status="${s.id}" class="${s.id === item.status ? 'on' : ''}" title="${s.hint}" style="--c:${STATUS_COLOR[s.id]}">${s.label}</button>`,
      ).join('')}</div>
      <label class="el-field"><span>Height</span>
        <input type="number" id="el-height" min="0" max="260" step="5" value="${h ?? ''}" ${h === null ? 'disabled' : ''} /> <span class="unit">cm</span>
      </label>
      <label class="check"><input type="checkbox" id="el-ceil" ${h === null ? 'checked' : ''} /> In the ceiling</label>
      <label class="el-field note"><span>Note</span><input type="text" id="el-note" maxlength="200" placeholder="e.g. over benk, bak TV" value="${escapeHtml(item.note ?? '')}" /></label>
      <div class="el-pos">x ${item.x.toFixed(2)} · y ${item.y.toFixed(2)} m</div>
      <button class="danger" id="el-del">Delete point</button>
    `;
