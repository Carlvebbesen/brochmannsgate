...
import { bboxCenter, polygonGeometry, signedArea, slabGeometry, toWorld } from '../core/geom';
import type { BuildContext } from './context';

export function buildRooms(ctx: BuildContext) {
  for (const room of rooms) {
    addLabel(ctx, room.name, Math.abs(signedArea(room.polygon)), room.labelAt ?? bboxCenter(room.polygon), room.floorLevel ?? 0);
